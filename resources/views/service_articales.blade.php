...
<div class="articals py-5" id="articals">
    <div class="container">
        ...
        <div class="row justify-content-center">
            @foreach ($articales->contents as $articale)
                <div class="col-md-4 col-sm-6 col-12 mb-3">
                    <div class="card h-100">
                        <img src="{{$articale->getFirstMediaUrl('content')}}" class="w-100" alt="">
                        <div class="content p-2">
                            <h3>{{$articale->title}}</h3>
                            <strong>القسم</strong>
                            <p class="major">{{$articale->name}}</p>
                            <p>
                                {{$articale->intro}}
                            </p>
                        </div>
                        <div class="card-footer d-flex justify-content-between align-items-center">
                            <div class="date">
                                ...
                                <span>{{$articale->created_at->format('Y/m/d')}}</span>
                            </div>
                            <a href="{{route('articale.show',$articale->slug)}}">قراءة المقال<i class="fa-solid fa-arrow-left ms-1"></i></a>
                        </div>
                    </div>
                </div>
            @endforeach
